...
# ----- BitLoot Media BUCKET -----
/**
resource "aws_s3_bucket" "bitloot_media_bucket" {
  bucket = "${var.s3_bitloot_media_bucket_name}"
}

resource "aws_s3_bucket_intelligent_tiering_configuration" "bitloot_media_bucket_configuration" {
  bucket = aws_s3_bucket.bitloot_media_bucket.id
  name   = "BitLootMediaBucketTiering"
  tiering {
    # 5minutes to few hours access time depending on object size.
    access_tier = "ARCHIVE_ACCESS"
    days        = 700
...
resource "aws_s3_bucket_ownership_controls" "bitloot_media_bucket_ownership" {
  bucket = aws_s3_bucket.bitloot_media_bucket.id
  rule {
    object_ownership = "BucketOwnerPreferred"
  }
...
resource "aws_s3_bucket_acl" "bitloot_media_bucket_acl" {
  bucket = aws_s3_bucket.bitloot_media_bucket.id
  acl    = "public-read"
  
  depends_on = [
    aws_s3_bucket_ownership_controls.bitloot_media_bucket_ownership,
    aws_s3_bucket_public_access_block.bitloot_block_public_access,
  ]
}

data "aws_iam_policy_document" "s3_bitloot_media_bucket_policy" {
  policy_id = "s3_bitloot_media_bucket_read"
...
resource "aws_s3_bucket_policy" "s3_bitloot_bucket_policy_attach" {
  bucket = aws_s3_bucket.bitloot_media_bucket.id
  policy = data.aws_iam_policy_document.s3_bitloot_media_bucket_policy.json
}
*/
